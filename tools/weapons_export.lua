...
-- add to the bottom:
--
--   base.dofile("C:/Users/Martin/Desktop/dev/sdcs/miz-maker/scripts/weapons_export.lua")
--
...
local weapon_table_id = {}
local weapon_table_by_name = {}
...
function add_to_weapon_db(ws_type, weapon_data)
	...
	for x=1,3 do
		...
		if dest[key] == nil then dest[key] = {} end
		dest = dest[key]
	end
...
function get_warehouse_category(ws)
	...
	if ws[1] == wsType_Air and ws[2] == wsType_Free_Fall and ws[3] == wsType_FuelTank then return "Fuel Tank" end

	-- Next we know everything should have 4, if not don't assign to a filter
	if ws[1] ~= wsType_Weapon then return nil end

	-- Missiles
	if ws[2] == wsType_Missile then
		if ws[3] == wsType_AA_Missile then return "AA Missiles" end
		if ws[3] == wsType_AA_TRAIN_Missile then return "AA Missiles" end

		if ws[3] == wsType_AS_Missile then return "AG Missiles" end
		if ws[3] == wsType_AS_TRAIN_Missile then return "AG Missiles" end
	end

	-- Rockets are simple
	if ws[2] == wsType_NURS then
		if ws[3] == wsType_Rocket then return "Rockets" end
	end

	-- Bombs if they're guided, guided, else bomb
	if ws[2] == wsType_Bomb then
		if ws[3] == wsType_Bomb_Guided then return "Guided Bombs" end
		return "Bombs"
	end

	-- Pods under Pods
	if ws[2] == wsType_GContainer then return "Pods" end
...
function add_launcher_to_export(airframe, pylon, launcher)
	...
	for weapon_ws_type_str, weapon_count in pairs(launcher_data.evaluated) do
		...
		if export_weapons[weapon_ws_type_str] == nil then
			...
			if weapon_data == nil then
				...
				export_weapons[weapon_ws_type_str] = {
	...
	export_launchers[launcher.CLSID] = {
...
function export_aircraft(airframe)
		...
		for radio_id, radio_data in pairs(airframe.panelRadio) do
			if radio_data.range ~= nil then
				...
				for _, channel in ipairs(radio_data.channels) do
					...
					if channel.modulation == "FM" then modulation_id = 1 end
	...
	export_aircraft_data[airframe.type] = {
...
function process_launcher_item(launcher)
	...
	if launcher.evaluated ~= nil then return launcher.evaluated end
		...
		for _, element in ipairs(launcher.Elements) do
			if element.payload_CLSID ~= nil and launchers[element.payload_CLSID] ~= nil then
				...
				for weapon, count in pairs(element_info) do
					...
					if weapons[weapon] == nil then weapons[weapon] = 0 end
...
-- Now we have our initial weapons db, save it just for reference

local file, error = io.open(lfs.writedir()..[[weapons_db_pre.json]], 'w')
if error then
	log.write('ERRR', log.INFO, tostring(error))
else
	file:write(JSON:encode({
		["weapons"] = weapons_db,
		["aliases"] = weapons_aliases,
	}))
	file:close()
end
...
--dump(rockets, "rockets")
--dump(bombs, "bombs")

-- create weapons_by_name lookup -> weapons

--dump(weapon_table_id, "weapon_table_id")
--dump(db, "db")
--dump(weapons_table, "weapons_table")
--dump(weapon_table['LAU_88_AGM_65H_2_L'])
--dump(AGM_65H, "AGM65H")

--dump(export_aircraft_data, "export_aircraft_data")

local file, error = io.open(lfs.writedir()..[[mdc_airframe.json]], 'w')
if error then
	log.write('ERRR', log.INFO, tostring(error))
else
	file:write(JSON:encode({
		['launchers'] = export_launchers,
		['airframes'] = export_aircraft_data,
		['weapons'] = export_weapons,
	}))
	file:close()
end

-- Dump all our weapons too
local full_weapons_export = {}
function dump_all_weapons(level)
	for k, weapon_data in pairs(level) do
		if weapon_data ~= nil then
			if weapon_data.ws_type_str ~= nil then
				full_weapons_export[weapon_data.ws_type_str] = {
					['ws_type'] = weapon_data.ws_type_str,
				...
				dump_all_weapons(weapon_data)
			end
		end
	end
end
dump_all_weapons(weapons_db)


file, error = io.open(lfs.writedir()..[[weapon_aliases.json]], 'w')
if error then
	log.write('ERRR', log.INFO, tostring(error))
else
	file:write(JSON:encode(weapons_aliases))
	file:close()
end


file, error = io.open(lfs.writedir()..[[full_weapons_export.json]], 'w')
if error then
	log.write('ERRR', log.INFO, tostring(error))
else
	file:write(JSON:encode(full_weapons_export))
	file:close()
end

file, error = io.open(lfs.writedir()..[[weapon_table_id.json]], 'w')
if error then
	log.write('ERRR', log.INFO, tostring(error))
else
	file:write(JSON:encode(weapon_table_id))
	file:close()
end
